...
export default function RCTrainer() {
  ...
  const updateThrottle = (clientY) => {
    ...
    if (Math.abs(value) < 5) setThrottle(0);
    else setThrottle(value);
  };

  const updateSteering = (clientX, clientY) => {
    ...
    const value = Math.max(-50, Math.min(50, (dx / radius) * 50));

    if (Math.abs(value) < 2) setSteering(0);
    else setSteering(value);
  };
  ...
  useEffect(() => {
    ...
    const updatePhysics = () => {
      setCar(prev => {
        const accel = throttle / 3600;
        const turnAccel = steering / 900;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4 select-none touch-none">
      ...
      <div className="relative w-64 h-96 bg-green-800 rounded-2xl mb-6 overflow-hidden border-2 border-green-600">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-8 border-2 border-white rounded-sm bg-green-900 opacity-40" />
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-24 h-8 border-2 border-white rounded-sm bg-green-900 opacity-40" />
      ...
      <div className="grid grid-cols-2 gap-10">
        <div className="flex flex-col items-center">
          ...
          <div
            ...
            onPointerMove={(e) => e.pressure > 0 && updateSteering(e.clientX, e.clientY)}
          ...
          >
            ...
            <motion.div
              className="absolute w-10 h-10 bg-gradient-to-b from-blue-400 to-blue-600 rounded-full shadow-lg"
              animate={{ x: steering }}
        ...
        <div className="flex flex-col items-center">
          ...
          >
            ...
            <motion.div
              className="absolute w-full h-6 bg-gradient-to-b from-green-400 to-green-600 rounded-md shadow-lg"
              animate={{ y: -throttle }}
              transition={{ type: "tween", duration: 0 }}
